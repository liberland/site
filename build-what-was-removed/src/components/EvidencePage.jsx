    ...
    <section className="section section--tight">
      ...
      <div style={{ border: "1px solid var(--rule-strong)", background: "#fff", marginBottom: 64 }}>
        {window.Data.grades.map((g) => (
          <div key={g.key} style={{ display: "grid", gridTemplateColumns: ".8fr 1.6fr .6fr", gap: 24, padding: "18px 22px", borderBottom: "1px solid var(--rule)", alignItems: "baseline" }}>
      ...
      <div className="card-grid card-grid--2">
        <div>
          ...
          {window.Data.approvedTerms.map((t) => (
            <div key={t} style={{ fontFamily: "var(--sans)", fontSize: 14, padding: "9px 0", borderTop: "1px solid var(--rule)" }}>{t}</div>
          ))}
        </div>
        <div>
          ...
          {window.Data.prohibitedTerms.map((t) => (
            <div key={t} style={{ fontFamily: "var(--sans)", fontSize: 14, padding: "9px 0", borderTop: "1px solid var(--rule)", color: "var(--muted-2)", textDecoration: "line-through" }}>{t}</div>
          ))}
        </div>
      </div>
    </section>
